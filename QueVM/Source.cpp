...
int main(int argc, const char** argv) {
	...
	if (argc > 1) {
		...
		if (input.is_open()) {
			...
			char* instructions = new char[fileSize];

			input.read((char*)instructions, fileSize);

			int programStart = (vmMemorySize - fileSize - 4) & 0xFFFFFFFC;
			int stackStart = programStart - 40;
			vm.loadProgram((uint8_t*)instructions, fileSize, programStart);

			vm.executeProgram(programStart, stackStart);
